...
update_cpu_panel() {
    ...
    local usage=$(echo "$top" | grep -oP '%Cpu\(s\).*id,\s*\K[0-9.]+' | awk '{print 100-$1}' || echo "0")
    usage=${usage:-0}
...
update_memory_panel() {
    local m=($(free -m | awk '/Mem:/ {print $2,$3,$4,$6,$7} /Swap:/ {print $3,$2}' || echo "0 0 0 0 0 0 0"))
...
update_disk_panel() {
    ...
    local root_used_pct=$(echo "$df_out" | awk '{print $5}' | tr -d '%' || echo "0")
    ...
    if command -v iostat >/dev/null 2>&1; then
        read rd wr <<< $(iostat -d -k 1 2 | tail -1 | awk '{print $3,$4}' || echo "N/A N/A")
    fi
...
update_network_panel() {
    local stats=($(grep -w "${NET_DEV}:" /proc/net/dev | awk '{print $2,$10}' || echo "0 0"))
    local rx=${stats[0]:-0} tx=${stats[1]:-0}
    ...
    if command -v ss >/dev/null 2>&1; then
        conns=$(ss -s | grep -oP 'TCP:\s*\K\d+(?=\s*\()' || echo "N/A")
    fi
...
update_services_panel() {
    ...
    for svc in "${SERVICES[@]}"; do
        local state="unknown"
        if systemctl is-active --quiet "$svc" 2>/dev/null; then state="active"
        elif systemctl is-failed --quiet "$svc" 2>/dev/null; then state="failed"
        elif systemctl is-active --quiet "$svc" >/dev/null 2>&1; then state="activating"
        fi
        
        local color=$GRAY dot="○"
        case $state in
            active) color=$GREEN; dot="$DOT" ;;
            activating) color=$YELLOW; dot="$DOT" ;;
            failed) color=$RED; dot="$DOT" ;;
        esac
        
        update_line $row $((RIGHT_COL+2)) "${svc^}: "
        update_line $row $((RIGHT_COL+2 + 12)) "${color}${dot} ${state^}${RESET} "
        ((row++))
    done
}
